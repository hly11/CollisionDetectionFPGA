...
entity image_filter_fh_control_bus_s_axi is
    generic (
...
end entity image_filter_fh_control_bus_s_axi;
...
-- 0x1c : reserved
-- 0x20 : Data signal of filter_size
--        bit 31~0 - filter_size[31:0] (Read/Write)
-- 0x24 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of image_filter_fh_control_bus_s_axi is
    ...
    constant ADDR_FILTER_SIZE_DATA_0 : INTEGER :=16#20#;
    constant ADDR_FILTER_SIZE_CTRL : INTEGER :=16#24#;
    ...
    signal int_filter_size     : UNSIGNED(31 downto 0);
...
begin
    ...
    begin
        if (ACLK'event and ACLK = '1') and ACLK_EN = '1' then
            if (ar_hs = '1') then
                ...
                when ADDR_FILTER_SIZE_DATA_0 =>
                    rdata_data <= RESIZE(int_filter_size(31 downto 0), 32);
                when others =>
                    rdata_data <= (others => '0');
                end case;
            end if;
    ...
    filter_size <= STD_LOGIC_VECTOR(int_filter_size);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FILTER_SIZE_DATA_0) then
                    int_filter_size(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_filter_size(31 downto 0));
                end if;
            end if;
        end if;
    end process;
