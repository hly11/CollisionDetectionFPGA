...
inline int xVDMA_Init(xVDMA_info *info, unsigned int baseAddr, int width, int height, int pixelLength)
{
...
inline void xVDMA_UnInit(xVDMA_info *info)
{
    munmap((void *)info->vdmaVirtualAddress, xVDMAMapLen);
    close(info->vdmaHandler);
}
 
inline unsigned int xVDMA_Get(xVDMA_info *info, int num)
{
...
inline void xVDMA_Set(xVDMA_info *info, int num, unsigned int val)
{
    if(num>=0 && num<xVDMACount)
    {
...
inline void xVDMA_kick2(xVDMA_info *info, unsigned int inadr)
{
   xVDMA_Set(info, 0x00/4, 1);
   xVDMA_Set(info, 0x5C/4, inadr);
   xVDMA_Set(info, 0x58/4, info->pixelLength*info->width);
   xVDMA_Set(info, 0x54/4, info->pixelLength*info->width);
   xVDMA_Set(info, 0x50/4, info->height);

   return;
}

   
inline void xVDMA_kick(xVDMA_info *info, unsigned int inadr, unsigned int outadr)
{
...
inline void xVDMA_OutStart(xVDMA_info *info, unsigned int adr, int circular)
{
...
inline void xVDMA_Start(xVDMA_info *info, unsigned int inadr)
{
...
inline int xVDMA_IsRunning(xVDMA_info *info)
{
    return (xVDMA_Get(info, 0x34/4)&1)==1;
}
 
inline int xVDMA_IsDone(xVDMA_info *info)
{
    return (xVDMA_Get(info, 0x34/4));
}
 
inline void xVDMA_Disp(xVDMA_info *info, char *str, int num)
{
